...
exports.json_file_name = exports.Config = exports.usage = exports.name = exports.inject = void 0;
exports.apply = apply;
...
exports.Config = koishi_1.Schema.object({
    ...
    help_model: koishi_1.Schema.boolean().default(true).description('是否开启图片使用说明'),
    ...
    input_time: koishi_1.Schema.number().default(60000).description('等待图片输入时间'),
...
exports.json_file_name = 'sms_studata_main_talk.json';
async function apply(ctx, config) {
    ...
    async function create_json() {
        ...
        await fmp.json_create(root, exports.json_file_name, [...smsdata, ...khrdata]);
    }
    await create_json();
    //审核配置
        ...
        for (let i = 0; i < 20; i++) {
            try {
                ...
                const fileChecks = pluass.map(async (i) => {
                    return await (0, FMPS_F_1.file_search)(`${root}/${i['Id_db']}.png`);
                });
...
🟢7.输入图片
    ...
    `;
        ...
        async function cal_arg1(arg1) {
            ...
            if (!arg1) {
                if (helpmod) {
                    return koishi_1.h.image(root + "talk_helptext.jpg");
                }
                else {
                    return help_text;
                }
        ...
        if (!arg1) {
            if (helpmod) {
                return koishi_1.h.image((0, url_1.pathToFileURL)(root + "/talk_helptext.jpg").href);
            }
            else {
                return help_text;
            }
        ...
        async function draw_ultra() {
            ...
            if (img_parr[0] > 0) {
                ...
                for (let i = 0; i < img_parr[0]; i++) {
                    const mess = (koishi_1.h.parse(await session.prompt(inp_time)));
                    if (mess[0].type == 'img') {
